  ...
  internal class CryptoTests {
    ...
    public async Task SealedBoxEncryption() {
      ...
      using (encKeyPairTuple.Item2) {
        ...
        Assert.That(async () => await session.Crypto.DecryptSealedBoxAsync(Utils.GetRandomData(10).ToList(), encKeyPairTuple.Item1, encKeyPairTuple.Item2), Throws.TypeOf<FfiException>());
    ...
    public async Task SignAndVerify() {
      ...
      using (signKeyPairTuple.Item2) {
        ...
        Assert.That(async () => await session.Crypto.VerifyAsync(Utils.GetRandomData(20).ToList(), signKeyPairTuple.Item1), Throws.TypeOf<FfiException>());
      }
    ...
    public async Task EncryptAndDecryptTest() {
      ...
      using (encKeyPairTuple.Item2) {
        ...
        Assert.That(decryptedBytes, Is.EqualTo(plainBytes));
        Assert.That(async () => await session.Crypto.DecryptAsync(Utils.GetRandomData(20).ToList(), encKeyPairTuple.Item1, encKeyPairTuple.Item2), Throws.TypeOf<FfiException>());
        Assert.That(async () => await session.Crypto.DecryptAsync(cipherBytes, encKeyPairTuple.Item1, encKeyPairTuple.Item1), Throws.TypeOf<FfiException>());
      }
